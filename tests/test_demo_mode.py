...
def test_demo_login_disabled_when_not_demo_mode(client, app):
    """POST /auth/demo-login/<persona> should 404 when DEMO_MODE is false."""
    app.config["DEMO_MODE"] = False
    resp = client.post("/auth/demo-login/brookes_student")
    assert resp.status_code == 404


def test_demo_login_success(client, app, db_session):
    """POST /auth/demo-login/brookes_student should authenticate persona and redirect to subdomain."""
    user = User(
        name="Aisha Rahman",
    ...
    user.set_password("BrookesDemo1")
    db_session.session.add(user)
    db_session.session.commit()

    app.config["DEMO_MODE"] = True
    try:
        resp = client.post("/auth/demo-login/brookes_student", follow_redirects=False)
...
def test_demo_login_invalid_persona(client, app):
    """POST /auth/demo-login/unknown_persona should flash error and redirect."""
    app.config["DEMO_MODE"] = True
    try:
        resp = client.post("/auth/demo-login/nonexistent", follow_redirects=True)
        assert resp.status_code == 200
        assert b"Invalid demo persona selected" in resp.data
    finally:
        app.config["DEMO_MODE"] = False


def test_login_page_renders_demo_personas_when_demo_mode(client, app):
    """GET /auth/login should display 1-click persona panel in demo mode."""
    app.config["DEMO_MODE"] = True
    try:
        resp = client.get("/auth/login")
        assert resp.status_code == 200
        assert b"Portfolio Showcase \xe2\x80\x94 1-Click Demo Login" in resp.data or b"Portfolio Showcase" in resp.data
    finally:
        app.config["DEMO_MODE"] = False
